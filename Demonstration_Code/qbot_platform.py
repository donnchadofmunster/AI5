...
class Keyboard():
    def __init__(self,ip='localhost'):
        ...
        self.k_up       = np.zeros((1), dtype = np.float64) # Additional Up button
        self.k_down       = np.zeros((1), dtype = np.float64) # Additional Down button
        self.k_left       = np.zeros((1), dtype = np.float64) # Additional Left button
        self.k_right       = np.zeros((1), dtype = np.float64) # Additional Right button
    ...
    def read(self):
        ...
        if self._handle.connected:
            ...
            if new:
                ...
                # self.k_left = self._handle.receiveBuffer[1]      # Additional Left button
                # self.k_right = self._handle.receiveBuffer[2]
                self.k_up = self._handle.receiveBuffer[12]
                self.k_down = self._handle.receiveBuffer[13]
                self.k_left = self._handle.receiveBuffer[15]  # Left Arrow (example index)
                self.k_right = self._handle.receiveBuffer[14] # Right Arrow (example index)
                # print(self._handle.receiveBuffer)  # Debug: print entire buffer
