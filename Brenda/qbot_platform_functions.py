...
def store_data(filename, value):
    """Appends a new value to a CSV file."""
    with open(filename, 'a', newline='') as f:
        writer = csv.writer(f)
        writer.writerow([value])  # Append a single value as a new row
...
class QBPVision():
    ...
    def subselect_and_threshold(self, image, rowStart, rowEnd, minThreshold, maxThreshold):
        ...
        thresholding."""
        store_image('images2', image)
        #------------Replace the following lines with your code---------------#
        subImage = image[rowStart:rowEnd,:]
        ret, binary = cv2.threshold(subImage, minThreshold, maxThreshold, cv2.THRESH_BINARY)
        #---------------------------------------------------------------------#
    ...
    def image_find_objects(self, image, connectivity, minArea, maxArea):
        ...
        (labels, ids, values, centroids) = cv2.connectedComponentsWithStats(image, 4, cv2.CV_32S)
        #---------------------------------------------------------------------#
    ...
    #             error = 160 - col
    #             # Generate a unique filename using date and time
    #             store_data("error.csv", error)
    #             angle = np.arctan2(error, 75)
    #             print(f'col: {col}')
    #             print(f'angle: {angle}')
    #             turnSpd = kP * angle + kD * derivative.send(angle)
    #             print(f'turnSpd: {turnSpd}')
    #             # Change the forward speed to make it easier to see the impact of
    #             # control constants on the turn speed
    ...
    def line_to_speed_map(self, sampleRate, saturation):
        ...
        while True:
            prediction = yield forSpd, turnSpd
            if categories[prediction] == 'slight_left':
                turnSpd = 0.25
            elif categories[prediction] == 'left':
                turnSpd = 0.125
            elif categories[prediction] == 'center':
                turnSpd = 0.0
            elif categories[prediction] == 'slight_left':
                turnSpd = -0.125
            elif categories[prediction] == 'right':
                turnSpd = -0.25
